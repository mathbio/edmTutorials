...
Now let's create the shadow manifold for both X and Y:
...
We are going to use the nearest neighbours of x(70) within the manifold Mx to predict y(70). To do this, we first create a matrix of distances among the states of Mx and then we select the three **[[WHY THREE?]]** nearest neighbours within the Mx:
...
Now we can expand the idea of predicting a single state of Y based on X, to predicting all the states of Y using X. If then do this prediction for every single value of Y, we will have a vector of the predicted states of Y (using X) and the *real* observed states of Y. We can then compare the predicted and observed:
