...
the bi-dimensional harmonic oscillator:
...
freedom, $x(t)$ and $y(t)$. This system has a circular attractor, that
...
And now you can create a lagged variable data frame in R by copying the following code and
...
is up to 2D + 1, where D is the dimensionality of the 
...
In this section, we check how robust the attractor is to error by adding simulated noise to the original time series.
...
attractor.  In the following days we will learn some methods to find
the optimal dimensions of the manifold. By now the take-home message
is: the more complex the attractor is, the more dimensions you
...
In this exercise toy have to to
