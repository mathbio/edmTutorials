...
You can also pan and rotate the manifold plot to get a better idea of
...
It is important to notice that the neighboring points in the manifold plot (above) correspond to the states of the system most similar to the focal state we want to predict. Below you can see each of these states in the time series, represented by the same code color used in the manifold plot above. 
...
the shadow manifold properly and thus worsen the forecast.
To find out the optimal embedding dimension, we can plot the forecast skill (correlation) against the embedding dimension. The optimal number of dimensions for our the current case is three (as we guessed
